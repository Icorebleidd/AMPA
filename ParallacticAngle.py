...
def calcoloParallattico(hAngles, declination, latitude):
    tan_nu = math.sin(hAngles) / ((math.tan(latitude) * math.cos(declination)) - (math.sin(declination) * math.cos(hAngles)))
    nu = math.atan(tan_nu)
...
def main():
    ...
    nu_values = [[calcoloParallattico(ang, dec, lat_radianti) for ang in angoli_resolved] for dec in declination_resolved]
    ...
    table_data = [[round(value, 2) for value in row] for row in nu_values]
